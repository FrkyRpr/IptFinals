...
//gcc -o receive.o receive.c -lsqlite3 -ljansson  -lfcgi
...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "username", json_string((const char*)sqlite3_column_text(stmt, 1)));
            json_object_set_new(object, "address", json_string((const char*)sqlite3_column_text(stmt, 2)));
            json_object_set_new(object, "email", json_string((const char*)sqlite3_column_text(stmt, 3)));
            json_object_set_new(object, "phone", json_string((const char*)sqlite3_column_text(stmt, 4)));
            json_object_set_new(object, "age", json_string((const char*)sqlite3_column_text(stmt, 5)));
            json_object_set_new(object, "gender", json_string((const char*)sqlite3_column_text(stmt, 6)));
            json_object_set_new(object, "lang", json_string((const char*)sqlite3_column_text(stmt, 7)));
            json_object_set_new(object, "msg", json_string((const char*)sqlite3_column_text(stmt, 8)));
    ...
    response_data = json_dumps(res_root,  JSON_INDENT(6)); //JSON_INDENT(4));
