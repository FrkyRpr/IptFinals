...
//gcc -o send.o send.c -lsqlite3 -ljansson  -lfcgi
...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "username", json_string(info.username));
    json_object_set_new(obj, "address", json_string(info.address));
    json_object_set_new(obj, "email", json_string(info.email));
    json_object_set_new(obj, "phone", json_string(info.phone));
    json_object_set_new(obj, "age", json_string(info.age));
    json_object_set_new(obj, "gender", json_string(info.gender));
    json_object_set_new(obj, "lang", json_string(info.lang));
    json_object_set_new(obj, "msg", json_string(info.msg));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id","username", "address", "email", "phone", "age", "gender", "lang", "msg"};
    char** infovar[] = {&info->id,&info->username,&info->address,&info->email,&info->phone,&info->age,&info->gender,&info->lang,&info->msg};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    dbres = sqlite3_bind_text(stmt, 2, info.username, -1, SQLITE_TRANSIENT);
    // bitwise operator
    dbres |= sqlite3_bind_text(stmt, 3, info.address, -1, SQLITE_TRANSIENT); 
    dbres |= sqlite3_bind_text(stmt, 4, info.email, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 5, info.phone, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.age, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 7, info.gender, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 8, info.lang, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 9, info.msg, -1, SQLITE_TRANSIENT);
